...
setwd("/Users/andrewzhang/Desktop/gerstein/wearableSensorData/Diabetes/")
...
x1 <- as.numeric(ag_daily_variance$IOB.Variance) # use this
x2 <- as.numeric(ag_daily_variance$HRV.Variance)
x3 <- as.numeric(ag_daily_variance$StepCount.Variance)
x4 <- as.numeric(ag_daily_variance$ActiveEnergyBurned.Variance)
x5 <- as.numeric(ag_daily_variance$BasalEnergyBurned.Variance)
...
x7 <- as.numeric(ag_daily_variance$HeartRate.Variance)
x8 <- as.numeric(ag_daily_variance$FlightsClimbed.Variance)
cor(cbind(y, x1, x2, x3, x4, x5, x6, x7, x8)[-86, ]) # max cor is -0.173
lm1 <- lm(y~x1+x2+x3+x4+x5+x6+x7+x8) # R-squared 0.06611

y <- as.numeric(glucose_in_target$PercentGlucoseInTargetRange)
x1 <- as.numeric(ag_daily_mean$IOB) # significant
...
cor(cbind(y, x1, x2, x3, x4, x5, x6, x7, x8)[-86, ]) # max cor is -0.452
lm2 <- lm(y~x1+x2+x3+x4+x5+x6+x7+x8) # R-squared 0.1976
...
x6 <- as.numeric(ag_daily_min$DistanceWalkingRunning) # cor = -0.167
x7 <- as.numeric(ag_daily_min$HeartRate)
x8 <- as.numeric(ag_daily_min$FlightsClimbed)
cor(cbind(y, x1, x2, x3, x4, x5, x6, x7, x8)[-86, ]) # max cor is -0.168
lm3 <- lm(y~x1+x2+x3+x4+x5+x6+x7+x8) # R-squared 0.0841
...
cor(cbind(y, x1, x2, x3, x4, x5, x6, x7, x8)[-86, ]) # max cor is -0.170
lm4 <- lm(y~x1+x2+x3+x4+x5+x6+x7+x8) # R-squared 0.08909

y <- as.numeric(glucose_in_target$PercentGlucoseInTargetRange)
x1 <- as.numeric(ag_daily_range$IOB)
x2 <- as.numeric(ag_daily_range$HRV)  # significant
...
cor(cbind(y, x1, x2, x3, x4, x5, x6, x7, x8)[-86, ]) # max cor is -0.266
lm5 <- lm(y~x1+x2+x3+x4+x5+x6+x7+x8) # R-squared 0.1052, HRV is slightly significant
...
cor(cbind(y, x1, x2, x3, x4, x5, x6, x7, x8)[-86, ]) # max cor is -0.454
lm6 <- lm(y~x1+x2+x3+x4+x5+x6+x7+x8) # R-squared 0.2493, nothing else significant
