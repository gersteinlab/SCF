...
```{r}
cml.pre <- cumsum(init.obs$t.ser)
end.pre <- max(nohop.pre)

cml.up <- c(cml.pre, cumsum(upper.confint) + cml.pre[end.pre])
cml.down <- c(cml.pre, cumsum(low.confit) + cml.pre[end.pre])
cml.pred <- c(cml.pre, cumsum(pred.val) + cml.pre[end.pre])
cml.obs <- c(cml.pre, cumsum(obs.val) + cml.pre[end.pre])
...
cml.df <- as.data.frame(cbind(cml.up, cml.down, cml.pred, cml.obs, x = 1:349)) %>%
...
cml.df %>%
  ...
  ggsave("../figures/cmlint_no-noise.png")
```
...
cml.df <- as.data.frame(cbind(cml.up, cml.down, cml.pred, cml.obs, x = 1:204)) %>%
  mutate(cml.effect = cml.obs - cml.pred,
         cml.effect.lower = cml.obs -cml.up,
         cml.effect.upper = cml.obs - cml.down)
...
cml.df %>%
  ...
  geom_path() +
  geom_ribbon(aes(ymin = cml.effect.lower, ymax = cml.effect.upper), fill = "darkblue", alpha = 0.5) +
  ggsave("../figures/cmlint_add-noise.png")
```
...
cml.df <- as.data.frame(cbind(cml.up, cml.down, cml.pred, cml.obs, x = 1:203)) %>%
  mutate(cml.effect = cml.obs - cml.pred,
         cml.effect.lower = cml.obs -cml.up,
         cml.effect.upper = cml.obs - cml.down)
...
cml.df %>%
  ...
  geom_path() +
  geom_ribbon(aes(ymin = cml.effect.lower, ymax = cml.effect.upper), fill = "darkblue", alpha = 0.5) +
  ggsave("../figures/cmlint_account-noise.png")
```
